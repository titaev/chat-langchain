...
run_prod:
	docker run --restart always --network host --name aii_chat_backend --env-file /etc/aii/aii_chat_backend.env -v /etc/aii/aii_chat_backend.env:/app/.env -v /var/log/aii/:/app/log -d  dextr/aii_admin_backend:chat_backend21
build:
